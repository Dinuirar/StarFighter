...
CShip::CShip(QString MODEL, weaponID WEAPONID, int ID,
    ...
    : CInertion(X, Y, ANGLE), id(ID), modelName(MODEL),
      ...
      energy(ENERGY), shields (SHIELDS), hull(HULL), we_id(WEAPONID), counterShip(0) {
...
//bool validate(int inertionCounter, int prev_step, CWeapon engage) {
//    if (inertionCounter - prev_step < engage->delay())
//        return false;
//    else
//        prev_step = inertionCounter;
//    if( energy - engage->energyCost() < 0 ) return false;
//    return true;
//}
